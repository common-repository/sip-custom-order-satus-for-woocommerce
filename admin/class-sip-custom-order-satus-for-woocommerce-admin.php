<?php

/**
 * The admin-specific functionality of the plugin.
 *
 * @link       https://shopitpress.com/
 * @since      1.0.0
 *
 * @package    Sip_Custom_Order_Satus_For_Woocommerce
 * @subpackage Sip_Custom_Order_Satus_For_Woocommerce/admin
 */

define( 'SIP_COSWC_UTM_CAMPAIGN', 'sip-custom-order-satus-for-woocommerce' );
define( 'SIP_COSWC_ADMIN_VERSION' , '1.0.4' );

if ( ! defined( 'SIP_SPWC_PLUGIN' ) )
	define( 'SIP_SPWC_PLUGIN',  'SIP Social Proof for WooCommerce' );

if ( ! defined( 'SIP_SPWC_PLUGIN_URL' ) )
	define( 'SIP_SPWC_PLUGIN_URL',  'https://shopitpress.com/plugins/sip-social-proof-woocommerce/' );

if ( ! defined( 'SIP_FEBWC_PLUGIN' ) )
	define( 'SIP_FEBWC_PLUGIN', 'SIP Front End Bundler for WooCommerce' );

if ( ! defined( 'SIP_FEBWC_PLUGIN_URL' ) )
	define( 'SIP_FEBWC_PLUGIN_URL', 'https://shopitpress.com/plugins/sip-front-end-bundler-woocommerce/' );

if ( ! defined( 'SIP_RSWC_PLUGIN' ) )
	define( 'SIP_RSWC_PLUGIN',  'SIP Reviews Shortcode for WooCommerce' );

if ( ! defined( 'SIP_RSWC_PLUGIN_URL' ) )
	define( 'SIP_RSWC_PLUGIN_URL',  'https://shopitpress.com/plugins/sip-reviews-shortcode-woocommerce/' );

if ( ! defined( 'SIP_AENWC_PLUGIN' ) )
	define( 'SIP_AENWC_PLUGIN',  'SIP Advanced Email Notification For Woocommerce' );

if ( ! defined( 'SIP_AENWC_PLUGIN_URL' ) )
	define( 'SIP_AENWC_PLUGIN_URL',  'https://shopitpress.com/plugins/sip-advanced-email-notifications-for-woocommerce/' );

if ( ! defined( 'SIP_COSWC_PLUGIN' ) )
	define( 'SIP_COSWC_PLUGIN',  'SIP Custom Order Satus for WooCommerce' );

if ( ! defined( 'SIP_COSWC_PLUGIN_URL' ) )
	define( 'SIP_COSWC_PLUGIN_URL',  'https://shopitpress.com/plugins/sip-custom-order-satus-woocommerce/' );

if ( ! defined( 'SIP_CCWC_PLUGIN' ) )
	define( 'SIP_CCWC_PLUGIN',  'SIP Cookie Check for WooCommerce' );

if ( ! defined( 'SIP_CCWC_PLUGIN_URL' ) )
	define( 'SIP_CCWC_PLUGIN_URL',  'https://shopitpress.com/plugins/sip-cookie-check-woocommerce/' );

if ( ! defined( 'SIP_CAWC_PLUGIN' ) )
	define( 'SIP_CAWC_PLUGIN',  'SIP Cart Ajax Refresh' );

if ( ! defined( 'SIP_CAWC_PLUGIN_URL' ) )
	define( 'SIP_CAWC_PLUGIN_URL',  'https://shopitpress.com/plugins/sip-cart-ajax-refresh/' );

if ( ! defined( 'SIP_MIWC_PLUGIN' ) )
	define( 'SIP_MIWC_PLUGIN',  'SIP Mautic integration for WooCommerce' );

if ( ! defined( 'SIP_MIWC_PLUGIN_URL' ) )
	define( 'SIP_MIWC_PLUGIN_URL',  'https://shopitpress.com/plugins/sip-mautic-integration-wooCommerce/' );

if ( ! defined( 'SIP_WPGUMBY_THEME' ) )
	define( 'SIP_WPGUMBY_THEME','WPGumby' );

if ( ! defined( 'SIP_WPGUMBY_THEME_URL' ) )
	define( 'SIP_WPGUMBY_THEME_URL','https://shopitpress.com/themes/wpgumby/' );

$get_optio_version = get_option( 'sip_version_value' );
if( $get_optio_version == "" ) {
	add_option( 'sip_version_value', SIP_COSWC_ADMIN_VERSION );
}
if ( version_compare( SIP_COSWC_ADMIN_VERSION , $get_optio_version , ">=" ) ) {
	update_option( 'sip_version_value', SIP_COSWC_ADMIN_VERSION );
}

/**
 * The admin-specific functionality of the plugin.
 *
 * Defines the plugin name, version, and two examples hooks for how to
 * enqueue the admin-specific stylesheet and JavaScript.
 *
 * @package    Sip_Custom_Order_Satus_For_Woocommerce
 * @subpackage Sip_Custom_Order_Satus_For_Woocommerce/admin
 * @author     ShopItPress <hello@shopitpress.com>
 */
class Sip_Custom_Order_Satus_For_Woocommerce_Admin {

	/**
	 * The ID of this plugin.
	 *
	 * @since    1.0.0
	 * @access   private
	 * @var      string    $plugin_name    The ID of this plugin.
	 */
	private $plugin_name;

	/**
	 * The version of this plugin.
	 *
	 * @since    1.0.0
	 * @access   private
	 * @var      string    $version    The current version of this plugin.
	 */
	private $version;

	/**
	 * Initialize the class and set its properties.
	 *
	 * @since    1.0.0
	 * @param      string    $plugin_name       The name of this plugin.
	 * @param      string    $version    The version of this plugin.
	 */
	public function __construct( $plugin_name, $version ) {

		$this->plugin_name = $plugin_name;
		$this->version = $version;

		add_action( 'admin_menu', array( $this, 'sip_coswc_admin_menu' ) );
		add_action( 'admin_menu', array( $this, 'sip_coswc_config_menu' ), 20 );
		add_action( 'admin_menu', array( $this, 'sip_spwc_sip_extras_admin_menu' ), 2000 );
		add_filter( 'plugin_action_links_' . SIP_COSWC_BASENAME, array( $this, 'sip_coswc_action_links' ) );
		add_action( 'admin_init', array( $this, 'sip_coswc_register_settings' ) );

	}

	public function sip_coswc_register_settings() { // whitelist options
		register_setting( 'sip-coswc-register-settings', 'public_key_of_mautic_server' );
	}


/**
	* Plugin page menus.
	*
	* @since 1.0.0
	*/
	public function sip_coswc_action_links( $links ) {
		$plugin_links = array(
			'<a href="' . admin_url( 'admin.php?page=sip-custom-order-satus-settings' ) . '">' . __( 'Settings', 'sip-custom-order-satus-for-woocommerce' ) . '</a>'
			);
		$plugin_links[] = '<a target="_blank" href="https://shopitpress.com/docs/' .SIP_COSWC_PLUGIN_SLUG. '/?utm_source=wordpress.org&utm_medium=SIP-panel&utm_content=v'. SIP_COSWC_VERSION .'&utm_campaign='.SIP_COSWC_UTM_CAMPAIGN.'">' . __( 'Docs', 'sip-custom-order-satus-for-woocommerce' ) . '</a>';

		return array_merge( $links, $plugin_links );
	}

	/**
	* Registers the admin menu for managing the ShopitPress options.
	*
	* @since 1.0.0
	*/
	public function sip_coswc_admin_menu() {
		$icon_svg = 'data:image/svg+xml;base64,';
		//add_menu_page( $page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $position );
		$this->hook = add_menu_page(
			__( 'SIP Plugin Panel', 'sip-custom-order-satus-for-woocommerce' ),
			__( 'SIP Plugins', 'sip-custom-order-satus-for-woocommerce' ),
			'manage_options',
			'sip_plugin_panel',
			NULL,
			$icon_svg,
			62.25
			);

		// Load global assets if the hook is successful.
		if ( $this->hook ) {
		// Enqueue custom styles and scripts.
			add_action( 'admin_enqueue_scripts',  array( $this, 'sip_coswc_admin_tab_style' ) );
		}
	}

	/**
	* Register the stylesheets for the admin area.
	*
	* @since    1.0.0
	*/
	public function sip_coswc_admin_tab_style() {
		wp_register_style( 'sip_coswc_custom_css', esc_url( SIP_COSWC_URL .   'admin/assets/css/custom.css', false, '1.0.0' ) );
		wp_enqueue_style( 'sip_coswc_custom_css' );
	}


	/**
	* Loads assets for the settings page.
	*
	* @since 1.0.0
	*/
	public function sip_coswc_admin_assets() {

		wp_register_style( 'sip_coswc_layout', esc_url( SIP_COSWC_URL .   'admin/assets/css/layout.css', false, '1.0.0' ) );
		wp_enqueue_style( 'sip_coswc_layout' );
	}

	public function sip_coswc_remove_duplicate_submenu() {
		/* === Duplicate Items Hack === */
		remove_submenu_page( 'sip_plugin_panel', 'sip_plugin_panel' );
	}

	public function sip_coswc_config_menu() {
		global $parent;
		$args = array(
			'create_menu_page' => true,
			'parent_slug'	=> '',
			'page_title'	=> __( 'Custom Order Satus', 'sip-custom-order-satus-for-woocommerce' ),
			'menu_title'	=> __( 'Custom Order Satus', 'sip-custom-order-satus-for-woocommerce' ),
			'capability'	=> 'manage_options',
			'parent'		=> '',
			'parent_page'	=> 'sip_plugin_panel',
			'page'			=> 'sip_plugin_panel',
			);

		$parent = $args['parent_page'];

		if ( ! empty( $parent ) ) {
			// add_submenu_page( $parent , 'Custom Order Satus', 'Custom Order Satus', 'manage_options', 'sip-custom-order-satus-settings', array( $this, 'sip_coswc_settings_page' ) );
		} else {
			add_menu_page( $args['page_title'], $args['menu_title'], $args['capability'], $args['page'], array( $this, 'sip_coswc_admin_menu_ui' ), NULL , 62.25 );
		}
		/* === Duplicate Items Hack === */
		$this->sip_coswc_remove_duplicate_submenu();
	}

	/**
	* To avoide the duplication of ShopitPress Extras menue and run the latest sip panel
	*
	* @since 1.0.1
	*/
	public function sip_spwc_sip_extras_admin_menu() {
		global $parent;
		$get_optio_version = get_option( 'sip_version_value' );

		if ( version_compare( $get_optio_version , SIP_COSWC_ADMIN_VERSION , "<=" ) ) {

			if ( ! defined( 'SIP_PANEL_EXTRAS' ) ) {
				define( 'SIP_PANEL_EXTRAS' , TRUE);
				add_submenu_page( $parent , 'ShopitPress Extras', '<span style="color:#FF8080 ">ShopitPress Extras</span>', 'manage_options', 'sip-extras', array( $this, 'sip_coswc_admin_menu_ui' ) );
				add_action( 'admin_enqueue_scripts',  array( $this, 'sip_coswc_admin_assets' ) );
			}
		}
	}

	/**
	* Outputs the main UI for handling and managing addons, themes and licenses.
	*
	* @since 1.0.0
	*/
	public function sip_coswc_admin_menu_ui() { ?>
		<div class="wrap">
			<h2><?php _e('Shopitpress extras','sip-custom-order-satus-for-woocommerce'); ?></h2>
			<h2 class="nav-tab-wrapper">
				<a class="nav-tab<?php if ( !isset( $_GET['action'] ) ) echo ' nav-tab-active'; ?>" href="admin.php?page=sip-extras"><?php _e( 'Plugins', 'sip-custom-order-satus-for-woocommerce' ); ?></a>
				<a class="nav-tab<?php if ( isset( $_GET['action'] ) && 'themes' == $_GET['action'] ) echo ' nav-tab-active'; ?>" href="admin.php?page=sip-extras&amp;action=themes"><?php _e( 'Themes', 'sip-custom-order-satus-for-woocommerce' ); ?></a>
			</h2>
			<?php
			if ( ! isset( $_GET['action'] ) ) {
				include("partials/ui/plugin.php");
			} elseif ( 'themes' == $_GET['action'] ) {
				include("partials/ui/themes.php");
			}
			?>
		</div>
		<?php
	} // END menu_ui()


	/**
	 * Register the stylesheets for the admin area.
	 *
	 * @since    1.0.0
	 */
	public function enqueue_styles() {

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'assets/css/sip-custom-order-satus-for-woocommerce-admin.css', array(), $this->version, 'all' );

	}

	/**
	 * Register the JavaScript for the admin area.
	 *
	 * @since    1.0.0
	 */
	public function enqueue_scripts() {


		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'assets/js/sip-custom-order-satus-for-woocommerce-admin.js', array( 'jquery' ), $this->version, false );

	}

}
